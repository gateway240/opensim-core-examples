 ...
 * Author(s): Matt S. DeMers, Alex Beattie                                    *
 *                                                                            *
...
// Author:  Matt DeMers, Alex Beattie
...
int main()
{
    try {
        ...
        osimModel.setAuthors("Matt DeMers, Alex Beattie");
        ...
        OpenSim::Body* linkage2 = new OpenSim::Body("linkage2", linkageMass,
                linkageMassCenter, linkageMass*linkageInertia);
        linkage2->attachGeometry(cylinder.clone());
        ...
        Inertia blockInertia = blockMass*Inertia::brick(blockSideLength * blockWidth,
                blockSideLength, blockSideLength);
        OpenSim::Body* block = new OpenSim::Body("block", blockMass,
                blockMassCenter, blockInertia);
        block->attachGeometry(new Brick(SimTK::Vec3(0.05 * blockWidth, 0.05, 0.05)));
        ...
        Vec3 locationInBlock(-0.15, linkageLength, 0.0);

        PinJoint* ankle = new PinJoint("ankle",
                ground, locationInGround, orientationInGround,
                *linkage1, locationInChild, orientationInChild);

        SliderJoint* knee = new SliderJoint("knee",
                *linkage1, Vec3(0), Vec3(0, 0, SimTK::Pi/2),
                *linkage2, Vec3(0), Vec3(0, 0, SimTK::Pi/2));

        PinJoint* hip = new PinJoint("hip",
                *linkage2, locationInBlock, orientationInChild,
        ...
        // double knee_range[2] = {0, SimTK::Pi*2};
        // knee->updCoordinate().setName("q2");
        // knee->updCoordinate().setRange(knee_range);
        ...
        piston->setPointB(locationInBlock);
        piston->setOptimalForce(1.0);
        piston->setPointsAreGlobal(false);

        osimModel.addForce(piston);

        // Vec3 pointOnBodies(0);
        // PointToPointActuator* piston = new PointToPointActuator();
        // piston->setName("piston");
